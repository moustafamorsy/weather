async function search(area) {
    let t = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=7d77b96c972b4d119a3151101212704&q=${area}&days=3`);
    if (t.ok && 400 != t.status) {
        let area = await t.json();
        displayCurrent(area.location, area.current),
        displayAnother(area.forecast.forecastday)
    }
}
document.getElementById("search").addEventListener("keyup", area=>{
    search(area.target.value)
}
);
